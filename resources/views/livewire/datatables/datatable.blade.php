        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mb-5">
            <div class="h-auto w-full mb-1">
                @if($this->searchableColumns()->count())
                    <div class="flex rounded-lg w-10/12 shadow-sm">
                        <div class="relative flex-grow focus-within:z-10">
                            ...
                            <input wire:model.debounce.500ms="search" class="block w-full py-3 pl-10 text-sm border-gray-300 leading-4 rounded-md shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 focus:outline-none" placeholder="{{__('Busca en')}} {{ $this->searchableColumns()->map->label->join(', ') }}" type="text" />
            ...
            @if($this->activeFilters)
                <span class="text-xl text-blue-400 uppercase">@lang('FILTRO ACTIVO')</span>
            @endif
            ...
            <div class="flex flex-wrap items-center space-x-1">
                ...
                @if($this->activeFilters)
                    <button wire:click="clearAllFilters" class="flex items-center px-3 text-xs font-medium tracking-wider text-red-500 uppercase bg-white border border-red-400 space-x-2 rounded-md leading-4 hover:bg-red-200 focus:outline-none"><span>{{ __('Reiniciar') }}</span>
                ...
                @if($exportable)
                    <div x-data="{ init() {
                        ...
                        <button wire:click="export" class="flex items-center px-3 text-xs font-medium tracking-wider text-green-500 uppercase bg-white border border-green-400 space-x-2 rounded-md leading-4 hover:bg-green-200 focus:outline-none"><span>{{ __('Exportar') }}</span>
        ...
        <div wire:loading.class="opacity-50" class="rounded-lg @unless($complex || $this->hidePagination) rounded-b-none @endunless shadow-lg bg-white max-w-screen overflow-x-scroll border-2 @if($this->activeFilters) border-blue-500 @else border-transparent @endif @if($complex) rounded-b-none border-b-0 @endif">
            ...
            @if($this->results->isEmpty())
                <p class="p-3 text-lg text-center">
                    {{ __("No hay nada que mostrar en este momento") }}
                </p>
            @endif
        </div>
                ...
                <div class="items-center justify-between p-2 sm:flex">
                    ...
                    @if(count($this->results))
                        <div class="flex items-center my-2 sm:my-0">
                            <select name="perPage" class="block w-full py-2 pl-3 pr-10 mt-1 text-base border-gray-300 form-select leading-6 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 sm:text-sm sm:leading-5 rounded-md" wire:model="perPage">
